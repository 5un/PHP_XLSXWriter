...
class XLSXWriter
{
	...
	protected function writeCell(XLSXWriter_BuffererWriter &$file, $row_number, $column_number, $value, $cell_format)
	{
		...
		if (!is_scalar($value) || $value=='') { //objects, array, empty
			...
			$file->write('<c r="'.$cell.'" s="'.$s.'" t="n"><v>0</v></c>'); // zero numeric string
		} elseif ( filter_var($value, FILTER_VALIDATE_INT) ){ 
			$file->write('<c r="'.$cell.'" s="'.$s.'" t="n"><v>'.($value).'</v></c>'); //int numeric string
		} elseif ( filter_var($value, FILTER_VALIDATE_FLOAT, FILTER_FLAG_ALLOW_THOUSAND) ){ 
			$value = str_replace(",", "", $value);
			$file->write('<c r="'.$cell.'" s="'.$s.'" t="n"><v>'.($value).'</v></c>');//float numeric string
		} elseif ($value{0}=='='){
